...
What's troublesome is not the annoying email itself, but Gmail's behavior: _it locked out my application until I manually confirmed its access._ This can't be a valid implementation, because then nobody can send emails from [willcarh.art]({{sys:home}}) while Gmail has it blocked! How do we get around this?
