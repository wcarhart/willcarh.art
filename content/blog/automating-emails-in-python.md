...
What's troublesome is not the annoying email itself, but Gmail's behavior: _it locked out my application until I manually confirmed its access._ This can't be a valid implementation, because then nobody can send emails from [willcarh.art]({{src:index.html}}) while Gmail has it blocked! How do we get around this?
...
In addition, I used this architecture for my own email utility, the Herald, for [willcarh.art]({{src:index.html}}). You can see it in action by going to the [contact]({{src:index.html}}) section of the homepage. If you'd like to take a peek at the [source code](https://github.com/wcarhart/willcarh.art-v1/blob/master/herald.py), please be my guest!
