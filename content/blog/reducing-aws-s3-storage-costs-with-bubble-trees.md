...
>> Wait just a second! | Wondering how we can print a complex bubble tree in a nice way? Check out my blog post for [building pretty trees on the command line]({{src:blog/how-to-print-file-trees-on-the-command-line.html}}), which lifts code directly out of the [bubble tree source](https://github.com/wcarhart/algos/blob/master/trees.py) in my [algos]({{src:project/algos.html}}) project.
...
If you'd like to take a look at bubble tree's code, check out my project [algos]({{src:project/algos.html}}), which has a bunch of standard and novel algorithms and data structure implementations in Python. You can also read [bubble tree's source code](https://github.com/wcarhart/algos/blob/master/trees.py), which contains other tree structures as well. If this is your first exposure to AWS, or you're looking to dive deeper, a great place to start is [AWS's tutorials](https://aws.amazon.com/getting-started/), where you can pick interactive tutorials based on what kind of developer you are (embedded, back-end, etc). Even though AWS was the pioneer of S3, nearly [every](https://cloud.google.com/storage) [other](https://www.digitalocean.com/products/spaces/) IaaS provider has a similar product, many of which work with AWS S3's API.
